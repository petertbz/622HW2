...
```{r}
cleaned_data = read.csv("D:/desktop/SURV/SECONDSEMESTER23-24/622/cleaned_data.csv")
```
...
# You can now create tables for counts by hour and by weekday
posts_by_hour <- table(cleaned_data$hour)
posts_by_weekday <- table(cleaned_data$weekday)

# To visualize the data by hour:
barplot(posts_by_hour, main="Number of Posts by Hour of Day", xlab="Hour of Day", ylab="Number of Posts", las=2)
```



```{r}
economy_clean$datetime <- as.POSIXct(economy_clean$timestamp, origin = "1970-01-01", tz = "UTC")

economy_clean$hour <- format(economy_clean$datetime, "%H")
economy_clean$weekday <- format(economy_clean$datetime, "%A")

posts_by_hour <- table(economy_clean$hour)
posts_by_weekday <- table(economy_clean$weekday)

daily_frequency <- economy_clean %>%
  dplyr::group_by(date) %>%
