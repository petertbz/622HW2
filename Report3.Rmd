...
```{r}
kable(listen, caption = "The number of comments from different subreddits")
```
...
The distribution of the sentiment of the posts is illustrated in Figure 1. The initial review of the hand-coded labels for each post showed that the majority expressed opposing sentiment to "economy is improving". More than half of the posts indicated that the reddit users felt that the economy was not faring well, and has been worse than the past times. On the contrary, analysis indicated that less than 20% of the posts had positive/favorable sentiments associated to them. 

```{r fig.cap="Distribution of in Favor (F), Neutral (N) and Opposing (O) sentiment of cleaned posts"}
ggplot(cleaned_data, aes(x = label)) +
  geom_bar() +
  theme_bw() +
  labs(x = "Label", y = "Count") +
  scale_x_discrete(labels = c("Favor", "Neutral", "Oppose"))
```

Furthermore, the distribution of time of the posts over the day was also analyzed. Figure 2 illustrated how most of the posts were made on the 2nd half of the day through to near mid-night. The discussions were less frequent over the span of the night. Moreover, it was observed that the most frequent posts were made from 3 PM to 6 PM. 

```{r fig.align='center', fig.cap="Distribution of posts made over the time of the day"}
ggplot(cleaned_data, aes(x = time_of_day)) +
  geom_histogram(stat = "count", binwidth = 1) +
  scale_x_continuous(breaks = seq(0, 23, 1), limits = c(0, 23)) +
  theme_bw() +
  labs(x = "Hour of Day", y = "Number of Posts")
```

After the initial meta-analysis, the posts were then reviewed to analyze the frequency of words to gather insights on specific topics that were being discussed in these forums. To do this, first the data was tokenized, and cleaned. Moreover, the common stop words were also removed with the use of tidytext library in R. It was observed that the most common words that appeared in the discussions were as expected from our initial assumption of the kind of discussions happening in the online forums. The frequency of the most common/frequent words occurring in the comments has been visualized as Figure 3. From the figure, we can see that the most common words were "people", "inflation", "money", "economy", "prices", "housing", "home", "market", "rates" and "tax". Most of the words above reflect the topics that people are most concerned about in the current economic situation.
...
```{r fig.align='center', fig.cap="Frequency of most commonly used words in the discussions"}
...
Figure 5 shows the number of each category by day. It is evident that the "Oppose" category has the highest number of comments on all three peak days, followed by the "Neutral" category. The "Favor" category has the lowest number of comments on all three days, indicating that the majority of the comments are negative or opposing the events. However, this phenomenon does not necessarily mean that the majority of the public is against these events. It could be that people who are against these events are more vocal and active in online forums like Reddit.

```{r fig.height=3, fig.cap="Composition of Comments by Day"}
groupdata = cleaned_data %>%
  group_by(date, label) %>%
  summarise(Frequency = n()) %>% 
...
groupdata$date <- format(groupdata$date, "%m-%d")

ggplot(groupdata, aes(x = date, y = Frequency)) +
  geom_bar(stat = "identity", aes(fill = label), position = "stack") +
  labs(x = "Date",
       y = "Number of Comments") + 
  theme_bw() +
...
Our project delves into Reddit discussions on the economy, revealing widespread concern over issues like inflation and housing. We found that people often post in the late afternoon, and significant news events tend to spark even more discussion. The data shows that despite some optimistic economic indicators, many people remain worried about the cost of living and their financial futures. However, the larger number of negative posts may also be "selection bias" in the social media. In general, this snapshot of public opinion shows us that economic issues are a major concern for many, and what happens in the economy is closely followed by the online community.
